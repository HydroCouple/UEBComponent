...
int WriteTSto3DNC( const char* FileName, const char* VarName, int dimOrd, int y_dim, int x_dim,int Nt_dim,  float* var_inp)  //ydim, xdim =the coordinate point the data to be written; Nt_dim =the length of the TS
{  
...
int create3DNC_uebOutputs(const char* FileName, const char* VarName, const char *varUnits, const char* tName, const char* tUnits, const char* tlong_name,
	const char* tcalendar, int Nt_dim, int dimOrd, float* t_inp, float *fillVal, const char* ws_FileName, const char* ws_VarName, const char* yName, const char* xName)
{	
...
int Create3DNC(const char* FileName, const char* VarName, const char *varUnits,  const char* tName,  const char* yName, const char* xName, const char* tUnits,
			  const char* yUnits, const char* xUnits, int Nt_dim, int Ny_dim, int Nx_dim, int dimOrd, float* t_inp,float* y_inp,float* x_inp, float *fillVal) 
{  
	// IDs for the netCDF file, dimensions, and variables. 
	int ncid = 0, y_dimid = 0, x_dimid = 0, t_dimid = 0; 
	int v_varid = 0, t_varid = 0, y_varid = 0, x_varid = 0; 
...
int Write3DNC(const char* FileName, const char* VarName, const char *varUnits,  const char* tName,  const char* yName, const char* xName, 
				...
				int Nt_dim, int Ny_dim, int Nx_dim, int dimOrd, float* t_inp,float* y_inp,float* x_inp,float*** var_inp) // int &t_dimid, int &y_dimid, int &x_dimid) //time, y, x
{  
	// IDs for the netCDF file, dimensions, and variables. 
	int ncid = 0, y_dimid = 0, x_dimid = 0, t_dimid = 0; 
	int v_varid = 0, t_varid = 0, y_varid = 0, x_varid = 0; 
...
int read3DNC(const char* FILE_NAME, const char* VAR_NAME, const char* xcor_NAME, 
			 const char* ycor_NAME, const char* tcor_NAME, float* xcorvar, float* ycorvar, float* tcorvar, float*** pvar_in)
{
	//ids for variable, axes,...
...
int readNC_TS(const char* FILE_NAME, const char* VAR_NAME, const char* tcor_NAME, const char* ycor_NAME,const char* xcor_NAME,
	               float* &pvar_in, float* &tcorvar,int ydim, int xdim, int &nrecords) //, int xstride) //double* tcorvar, double*** pvar_in)
{
	//6.24.14
	int xdimindx = 1;
...
int Write3DNC(const char* FILE_NAME, const char* VAR_NAME, const char* xcor_NAME, 
			 const char* ycor_NAME, const char* tcor_NAME, float* xcorvar, float* ycorvar, float* tcorvar, float*** pvar_out)
{   
	//ids for variable, axes,...
...
//the following code is intended to handle exeption when reading netcdf
//#_i needs revision to be used
/*int xceptHandled(int ncid, int ta_varid, float*** ta_inp)
{
	int retvalue = 0;
	__try
	{
		retvalue = nc_get_var_float(ncid, ta_varid, &ta_inp[0][0][0]);
	...
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
		...
		cout<<"Exception handler %lX\n", _exception_code()); 
		Sleep(2000);
		//DeleteMatrix(ta_inp, NREC, NLAT, NLON);
		//float*** ta_in= Create3DArray(NREC,NLAT,NLON);
		retvalue = xceptHandled(ncid, ta_varid, ta_inp);
	}
